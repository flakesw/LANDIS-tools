...
# I wrote this script to use FIA data organized how rFIA does it. That is, 
# inside the fia_folder, all the state data is listed without subfolders. 
# If you have it nested within subfolders, you should be able to modify
# the states argument to something like states = c("subfolder/CA", "subfolder/NV").
# I think that ought to work. 

preprocess_fia_data <- function(fia_folder = "D:/data/fia/rFIA_downloads/", 
                                states = c("CA", "NV"), 
                                ...
                                forest_ref_loc = "D:/Data/fia/FIADB_REFERENCE/REF_FOREST_TYPE.csv",
                                tree_output_loc = "fia_trees.csv",
                                plot_output_loc = "fia_plots.csv"){
  library(tidyverse)
  library(sf)
  
  forest_ref <- read.csv(forest_ref_loc)  
