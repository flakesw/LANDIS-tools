...
sp_ref <- read.csv("./REF_SPECIES.csv")
...
trees <- trees[trees$PLT_CN %in% plots_to_use$CN, ]
seedlings <- seedlings[seedlings$PLT_CN %in% plots_to_use$CN, ]
plot <- plot[plot$CN %in% plots_to_use$CN, ]

write.csv(trees %>% select(c(CN, PLT_CN, INVYR, SPCD, SPGRPCD, STATUSCD, DIA, HT, CARBON_AG)), "./example_data/trees.csv")
write.csv(filter(seedlings, SPCD %in% c(12, 746)), "./example_data/seedlings.csv")
write.csv(plot, "./example_data/plot.csv")
write.csv(filter(sitetrees, SPCD %in% c(12, 746)), "./example_data/sitetrees.csv")
