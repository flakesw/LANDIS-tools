...
out.dir<-"C:/Users/User/Documents/Postdoc/Beta Diversity/out/Beta Functional Group Output/MIROC/predators/"
...
##binarize each raster in stack and subset to predators
  preds<-pmatch(fnlg$Predators,grids)
  preds<-preds[!is.na(preds)]
  pgrids<-grids[preds]
  pgrids<-gsub("\\..*","",pgrids)
  pgridstack<-raster::subset(my_stack,pgrids)

##Convert raster stack to dataframe for betagrid function and create polygon to write results to
occurrences<-as.data.frame(pgridstack,xy=TRUE)
...
writeRaster(beta, paste0(out.dir,gsub(in.dir,"",scenarios[i]),"_predators_540_turnover"), format="GTiff", overwrite=TRUE)
...
writeRaster(beta, paste0(out.dir,gsub(in.dir,"",scenarios[i]),"_predators_540_nestedness"), format="GTiff", overwrite=TRUE)


##Extract the mean beta result and write/export to a rasterpoly2$betadiv <- results[,4]
emptyraster <- raster(extent(poly2))
res(emptyraster)=180
beta <- rasterize(poly2, field="betadiv", emptyraster)

writeRaster(beta, paste0(out.dir,gsub(in.dir,"",scenarios[i]),"_predators_540_meanbeta"), format="GTiff", overwrite=TRUE)
