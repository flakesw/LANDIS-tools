...
foreach(i = 1:length(scenarios),
  ...
  # binarize each raster in stack and subset to insectivores
  fg <- fnlg$Insectivores
  fg <- fg[fg != ""]
  ...
  terra::writeRaster(r, paste0(out.dir, gsub(in.dir,"",scenarios[i]),"_insectivores_540_turnover"), filetype="GTiff")
  
  ##Extract the turnover result and write/export to a raster
  r <- emptyraster
  r[results$cell] <- results$mean_nestedness
  terra::writeRaster(r, paste0(out.dir, gsub(in.dir,"",scenarios[i]),"_insectivores_540_nestedness"), filetype="GTiff")

  
  ##Extract the mean beta result and write/export to a raster
  r <- emptyraster
  r[results$cell] <- results$mean_beta
  terra::writeRaster(r, paste0(out.dir, gsub(in.dir,"",scenarios[i]),"_insectivores_540_beta"), filetype="GTiff")
