...
out.dir<-"C:/Users/User/Documents/Postdoc/Beta Diversity/out/Beta Functional Group Output/MIROC/insectivores/"
...
for (i in 1:length(scenarios)){
  ...
  insect<-pmatch(fnlg$Insectivores,grids)
  insect<-insect[!is.na(insect)]
  igrids<-grids[insect]
  igrids<-gsub("\\..*","",igrids)
  igridstack<-raster::subset(my_stack,igrids)

##Convert raster stack to dataframe for betagrid function and create polygon to write results to  
occurrences<-as.data.frame(igridstack,xy=TRUE)
...
writeRaster(beta, paste0(out.dir,gsub(in.dir,"",scenarios[i]),"_insectivores_540_turnover"), format="GTiff")
...
writeRaster(beta, paste0(out.dir,gsub(in.dir,"",scenarios[i]),"_insectivores_540_nestedness"), format="GTiff")
...
writeRaster(beta, paste0(out.dir,gsub(in.dir,"",scenarios[i]),"_insectivores_540_meanbeta"), format="GTiff")
