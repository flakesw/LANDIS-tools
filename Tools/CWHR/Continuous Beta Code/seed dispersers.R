...
foreach(i = 1:length(scenarios),
  ...
  terra::writeRaster(r, paste0(out.dir, gsub(in.dir,"",scenarios[i]),"_seeddispersers_540_turnover"), filetype="GTiff")
  
  ##Extract the turnover result and write/export to a raster
  r <- emptyraster
  r[results$cell] <- results$mean_nestedness
  terra::writeRaster(r, paste0(out.dir, gsub(in.dir,"",scenarios[i]),"_seeddispersers_540_nestedness"), filetype="GTiff")

  
  ##Extract the mean beta result and write/export to a raster
  r <- emptyraster
  r[results$cell] <- results$mean_beta
  terra::writeRaster(r, paste0(out.dir, gsub(in.dir,"",scenarios[i]),"_seeddispersers_540_beta"), filetype="GTiff")
